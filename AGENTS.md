...
- **Tasks completed**:
  ...
  - Implemented admin-controlled account approval system: new signups default to `is_approved=FALSE`, blocking login until an admin approves via the Hacker Panel. Added `is_approved` column to Profile model, Alembic migration (`f9b3c4d5e6f7`), auth middleware & login checks, admin approve endpoint (`POST /api/admin/users/{id}/approve`), and UI with PENDING/APPROVED badges and APPROVE/REJECT buttons.
  - Created an insulated query pre-filtering layer (`app/services/query_filter.py`) that intercepts greetings, profanity, and casual non-study chatter with regex pattern matching before they reach the Gemini API, returning a canned robotic response to conserve API calls.
  - Overhauled the dashboard greeting system with a 10-slot time-of-day schedule (midnight through late night) and Claude-style study-relatable taglines displayed below the greeting header.
  - Hidden the "Profile Settings" sidebar link for admin users since they have access to the Hacker Panel.
